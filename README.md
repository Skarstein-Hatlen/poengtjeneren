...
- Reise (`/no/reise`, `src/components/Reise.tsx`, data i `src/data/reise.json`): leiebil som gir EuroBonus-poeng per leie (Avis via EuroBonus-siden sin, Sixt) og forsinket fly etter EU 261/2004 (250, 400 eller 600 € etter distanse), med SAS' eget skjema (gratis) og flyforsinkelsestjenesten i landet (35 % hvis de vinner). Logoene lages av `npm run reiselogoer` (`public/logos/reise/`). `sjekk` i reise.json (side → tekster) kjøres daglig av `scripts/sjekk-kort.mjs`; Sixt stenger for skript og sjekkes for hånd. `annonse: true` merker lenken som annonse når en avtale er på plass.
