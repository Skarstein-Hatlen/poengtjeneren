...
/** Butikklisten fra lageret, hentet på nytt hvis den mangler eller er over et døgn gammel. */
async function butikker() {
  const { perDomene, oppdatert } = await chrome.storage.local.get(['perDomene', 'oppdatert']);
  if (perDomene && oppdatert && Date.now() - oppdatert < EN_DAG) return perDomene;
  return (await hentButikker()) ?? perDomene ?? null;
}
...
async function oppdaterMerke(tabId, url) {
  const butikk = finnButikk(await butikker(), domeneFor(url));
  try {
    if (!butikk) {
